            ...
            <div class="col-9">
                <div class="card" style="margin-top: 20px">
                    <div class="card-header">
                        ...
                        <!--<button type="button" class="btn btn-success float-end">創建Bot</button>-->
                                    ...
                                    <td>
                                        ...
                                        <div class="modal fade" :id="'update-bot-modal-' + bot.id" tabindex="-1">
                                            <div class="modal-dialog modal-xl">
                                                <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title fs-5" >修改Bot</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    <div class="mb-3">
                                                        <label for="add-bot-title" class="form-label">名称</label>
                                                        <input v-model = "bot.title" type="text" class="form-control" id="add-bot-title" placeholder="请输入Bot的名称">
                                                    </div>
                                                    <div class="mb-3">
                                                        <label for="add-bot-description" class="form-label">简介</label>
    ...
    setup(){
        ...
        const refresh_bots = () => {
            $.ajax({
                ...
                type : "get",
                headers : {   //凡是需要登录（不放行）才能访问的网页都需要header
            ...
            $.ajax({
                ...
                success(resp) {
                    if(resp.error_message === "success")
                    {
                        ...
                        botadd.description = "";
                        botadd.content = "";
                        Modal.getInstance("#add-bot-btn").hide();  //点击创建按钮时，编辑框退出
                        refresh_bots();
                    }
                    else {
        ...
        const update_bot = (bot) => {
            $.ajax({
                ...
                success(resp){
                    if(resp.error_message === "success")
                    {
                        Modal.getInstance('#update-bot-modal-' + bot.id).hide();  //点击创建按钮时，编辑框退出
                        refresh_bots();
                    }
                }
            })
